...
class CoursesTest(TestCase):
    ...
    def test_course_list_advanced_level(self):
        ...
        self.assertQuerysetEqual(
            advanced_level,
            response.context["advanced_level"],
            transform=lambda x: x,
            ordered=False,
        )
...
class CourseBooking(TestCase):
    ...
    def test_title(self):
        ...
        response = self.client.get(reverse("diving:course-booking-request"))
        self.assertEqual(response.context["title"], "PADI Course Booking Request")
    ...
    def test_diver_form(self):
        ...
        response = self.client.get(reverse("diving:course-booking-request"))
        self.assertEqual(response.context["diver_form"].__name__, formset.__name__)
    ...
    def test_invalid_data_post(self):
        ...
        response = self.client.post(reverse("diving:course-booking-request"), data)
        form = CourseBookingRequestForm()
        DiverFormSet = formset_factory(CourseBookingDivers)
        divers_formset = DiverFormSet()
        message = str(list(response.context["messages"])[0])
        self.assertEqual(message, "Sorry, your input was not valid, please try again")
        self.assertEqual(response.context["form"].__name__, form.__name__)
        self.assertEqual(
    ...
    def test_valid_object_input(self):
        ...
        response = self.client.post(reverse("diving:course-booking-request"), data)
        course_booking = CourseBookingRequest.objects.get(
            full_name=divers[0],
            email=data["email"],
            course=data["course"],
        )
